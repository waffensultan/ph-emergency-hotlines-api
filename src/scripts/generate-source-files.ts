    ...
    regions.forEach((region) => {
        ...
        provinces.forEach((province) => {
            ...
            cities_municipalities.forEach((city_municipality) => {
                generate_source_json([region, province, city_municipality], data);
            })
        })
    })
}
...
const generate_source_json = (params: string[], data: any) => {
    const source_json = params.reduce((acc, key) => (acc && acc[key] ? acc[key] : null), data);
    const file_path = '../models/data/' + params.map((param, index) => param + (index !== params.length - 1 ? "/" : "")).join(''); 
    ...
    if (source_json) {
        ...
        try {
            ...
            fs.writeFileSync(`${dir_path}/source.json`, JSON.stringify(source_json, null, 2))
        } catch (error) {
